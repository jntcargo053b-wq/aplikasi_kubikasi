...
class _BarangFormState extends State<_BarangForm> {
  ...
  Future<void> _showPhotoSourceSheet() async {
    final source = await showModalBottomSheet<ImageSource>(
      ...
      builder: (ctx) => SafeArea(
        child: Wrap(
          children: [
            ListTile(leading: const Icon(Icons.camera_alt_outlined), title: const Text('Ambil Foto'), onTap: () => Navigator.pop(ctx, ImageSource.camera)),
          ...
          child: Form(
            ...
            child: ListView(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                TextFormField(controller: _nama, decoration: const InputDecoration(labelText: 'Nama Barang'), validator: (v) => v == null || v.trim().isEmpty ? 'Wajib diisi' : null),
                const SizedBox(height: 12),
                TextFormField(controller: _jumlah, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Jumlah'), validator: (v) { final x = int.tryParse(v ?? ''); return x == null || x <= 0 ? 'Jumlah tidak valid' : null; }),
                const SizedBox(height: 12),
                TextFormField(controller: _berat, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Berat per unit', suffixText: 'kg'), validator: (v) => _d(v ?? '') < 0 ? 'Berat tidak valid' : null),
                const SizedBox(height: 12),
                Row(children: [Expanded(child: _size(_p, 'Panjang')), const SizedBox(width: 8), Expanded(child: _size(_l, 'Lebar')), const SizedBox(width: 8), Expanded(child: _size(_t, 'Tinggi'))]),
                const SizedBox(height: 14),
                Row(children: [Expanded(child: OutlinedButton.icon(onPressed: _busy ? null : _showPhotoSourceSheet, icon: const Icon(Icons.camera_alt_outlined), label: Text(_photo == null ? 'Foto Barang' : 'Ganti Foto'))), if (_photo != null) ...[const SizedBox(width: 10), GestureDetector(onTap: () => showPhotoPreview(context, _photo!), child: ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(File(_photo!), width: 54, height: 54, fit: BoxFit.cover, cacheWidth: 162, cacheHeight: 162, errorBuilder: (_, __, ___) => const SizedBox(width: 54, height: 54, child: Icon(Icons.broken_image_outlined)))))]),
                const SizedBox(height: 16),
                AnimatedBuilder(animation: _previewListenable, builder: (context, _) => Row(children: [Expanded(child: _metric('VOLUME TIMBANG', _volume.toStringAsFixed(2), '')), const SizedBox(width: 8), Expanded(child: _metric('KUBIKASI', _kubikasi.toStringAsFixed(3), 'm³'))])),
  ...
  Widget _size(TextEditingController c, String label) => TextFormField(controller: c, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: InputDecoration(labelText: label, suffixText: 'cm'), validator: (v) => _d(v ?? '') <= 0 ? 'Wajib' : null);
  ...
  Widget _metric(String title, String value, String suffix) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 10, color: AppColors.muted)), const SizedBox(height: 4), FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text('$value $suffix', maxLines: 1, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)))]),
  );
...
void showPhotoPreview(BuildContext context, String path) {
  Navigator.of(context).push(PageRouteBuilder(opaque: false, barrierColor: Colors.black87, pageBuilder: (_, __, ___) => _PhotoPreview(path: path)));
}
...
class _PhotoPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Scaffold(
      backgroundColor: Colors.transparent,
      body: SafeArea(child: Stack(children: [Center(child: InteractiveViewer(minScale: 1, maxScale: 4, child: Image.file(File(path)))), Positioned(top: 8, right: 8, child: IconButton(icon: const Icon(Icons.close, color: Colors.white, size: 28), onPressed: () => Navigator.of(context).pop()))]),
    ),
  );
}
